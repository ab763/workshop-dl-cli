...
describe('User config validations', () =>
{
	context('integer validation', () =>
	{
		test.it('should return true for integers', () =>
		{
			...
			expect(validations.integerValidation(1_000_000_000)).to.be.true;
			expect(validations.integerValidation(-5_000_000)).to.be.true;
		});
		...
		test.it('should return false for non integer data', () =>
		{
			...
			expect(validations.integerValidation(() =>
			{})).to.be.false;
			expect(validations.integerValidation(Number.NaN)).to.be.false;
	...
	context('concurrency validation', () =>
	{
		test.it('should reject all non integer values', async () =>
		{
			...
			expect(await validations.concurrencyValidation(() =>
			{})).to.be.false;
			expect(await validations.concurrencyValidation(Number.NaN)).to.be.false;
	...
	context('transferProgressTimeout validation', async () =>
	{
		test.it('should reject all non integer', async () =>
		{
			...
			expect(await validations.transferProgressTimeoutValidation(false)).to.be
				.false;
			expect(await validations.transferProgressTimeoutValidation(() =>
			{})).to
				.be.false;
			expect(await validations.transferProgressTimeoutValidation(Number.NaN)).to.be
				.false;
