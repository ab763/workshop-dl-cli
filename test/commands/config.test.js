...
describe('config <property>', () =>
{
	test
		...
		.it('should fetch saveDir', context =>
		{
			expect(context.stdout).to.contain(global.testUserConfig.path.saveDir);
		});

	test
		.stdout()
		.command(['config', 'tmpDir'])
		.it('should fetch tmpDir', context =>
		{
			expect(context.stdout).to.contain(global.testUserConfig.path.tmpDir);
		});
	test
		.stdout()
		.command(['config', 'concurrency'])
		.it('should fetch concurrency value', context =>
		{
			expect(context.stdout).to.contain(global.testUserConfig.process.concurrency);
		});
	test
		.stdout()
		.command(['config', 'transferProgressTimeout'])
		.it('should fetch transferProgressTimeout value', context =>
		{
	...
	test
		...
		.exit(2)
		.it('should exit without output for invalid property', context =>
		{
...
describe('config <property> <value>', () =>
{
	...
	test
		...
		.it('should set saveDir to any valid directory path', context =>
		{
			expect(global.userConfig.get('path.saveDir')).to.equal(__dirname);
			expect(context.stdout).to.contain(__dirname);
		});
	...
	test
		...
		.it('should not set saveDir to any invalid input', context =>
		{
			expect(global.userConfig.get('path.saveDir')).to.equal(
				global.testUserConfig.path.saveDir,
			);
			expect(context.stdout).to.not.contain('saveDir set to:');
		});

	test
		.stdout()
		.command(['config', 'saveDir', path.join(__dirname, 'testSample_1234')])
		.it('should not set saveDir to any non-existsing directory path', context =>
		{
			expect(global.userConfig.get('path.saveDir')).to.equal(
				global.testUserConfig.path.saveDir,
			);
			expect(context.stdout).to.contain('Invalid input for saveDir');
		});

	test
		.stdout()
		.command(['config', 'tmpDir', __dirname])
		.it('should set tmpDir to any valid directory path', context =>
		{
	...
	test
		...
		.it('should not set tmpDir to any invalid input', context =>
		{
			expect(global.userConfig.get('path.tmpDir')).to.equal(
				global.testUserConfig.path.tmpDir,
			);
			expect(context.stdout).to.not.contain('tmpDir set to:');
		});

	test
		.stdout()
		.command(['config', 'tmpDir', path.join(__dirname, 'testSample_1234')])
		.it('should not set tmpDir to any non-existsing directory path', context =>
		{
			expect(global.userConfig.get('path.tmpDir')).to.equal(
				global.testUserConfig.path.tmpDir,
			);
			expect(context.stdout).to.contain('Invalid input for tmpDir');
		});
	...
	test
		...
		.it(
			...
			context =>
			{
				...
				expect(context.stdout).to.not.contain('Invalid input for concurrency');
			},
		);
	...
	test
		...
		.it(
			...
			context =>
			{
				...
				expect(context.stdout).to.not.contain('concurrency set to:');
			},
		);
	...
	test
		...
		.it('should not set concurrency value to non integer values', context =>
		{
			expect(global.userConfig.get('process.concurrency')).to.equal(
				global.testUserConfig.process.concurrency,
			);
	...
	test
		...
		.it(
			...
			context =>
			{
				...
				expect(context.stdout).to.not.contain(
	...
	test
		...
		.it(
			...
			context =>
			{
				...
				expect(context.stdout).to.not.contain('transferProgressTimeout set to:');
			},
		);
